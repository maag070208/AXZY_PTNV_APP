...
/**
 * "Mis tareas": el backend ya filtra `/tickets/kanban` por el usuario en
 * sesión cuando su rol es EMPLEADO (o por su ámbito si es GERENTE/JEFE_DE_AREA),
 * así que esta pantalla y [AdminTasksScreen] reusan el mismo endpoint — la
 * diferencia de alcance la resuelve el servidor según el rol.
 */
@Composable
fun MyTasksScreen() {
    ...
    suspend fun load() {
        ...
        try {
            ...
            error = e.message ?: "No se pudieron cargar tus tareas"
        } finally {
            loading = false
        }
    }
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        Column(modifier = Modifier.padding(16.dp)) {
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                Text("${rows.size} tarea(s) asignada(s) a ti", style = MaterialTheme.typography.bodySmall, color = AppColors.TextMuted)
        ...
        when {
            ...
            rows.isEmpty() -> EmptyState("No tienes tareas asignadas", modifier = Modifier.weight(1f))
            else -> LazyColumn(
