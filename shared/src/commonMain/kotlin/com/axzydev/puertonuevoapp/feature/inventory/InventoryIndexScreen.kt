...
import com.axzydev.puertonuevoapp.core.network.InventorySummaryDto
...
fun InventoryIndexScreen() {
    ...
    var summary by remember { mutableStateOf<InventorySummaryDto?>(null) }
    ...
    suspend fun load() {
        ...
        error = null
        try {
            summary = AppContainer.inventoryApi.summary()
        } catch (e: Exception) {
            error = e.message ?: "No se pudo cargar el inventario"
        } finally {
            loading = false
        }
    }
    ...
    when {
        ...
        error != null || summary == null -> ErrorState(error ?: "Error", Modifier.fillMaxSize(), onRetry = { scope.launch { load() } })
        else -> {
            val s = summary!!
            Column(
                modifier = Modifier
                ...
                AppSurfaceCard(modifier = Modifier.fillMaxWidth()) {
                    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                        ...
                        Row(
                            ...
                            modifier = Modifier.clickable { navigator.push(Screen.LocationsList) },
                        ) {
                        ...
                        s.locations.take(5).forEach { loc ->
                            Row(
                                ...
                                Row(verticalAlignment = Alignment.CenterVertically) {
                                    ...
                                    Text(formatLocation(loc.lugar, loc.subLugar, loc.numero), style = MaterialTheme.typography.bodyMedium, color = AppColors.TextPrimary)
                                }
                ...
                AppSurfaceCard(modifier = Modifier.fillMaxWidth()) {
                    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                        ...
                        Row(
                            verticalAlignment = Alignment.CenterVertically,
                            modifier = Modifier.clickable { navigator.push(Screen.InventoryMovements) },
                        ) {
