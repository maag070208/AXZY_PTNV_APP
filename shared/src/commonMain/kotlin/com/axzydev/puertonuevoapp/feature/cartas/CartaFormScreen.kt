...
fun CartaFormScreen(cartaId: String? = null) {
    ...
    val viewModel: CartaFormViewModel = viewModel(key = "carta-form-${cartaId ?: "new"}") {
        CartaFormViewModel(cartaId, AppContainer.cartasApi, AppContainer.devicesApi, AppContainer.deviceTypesApi, AppContainer.usersApi)
    }
    ...
    val jefeOptions = state.filteredJefes.map { u ->
        u.id to listOfNotNull(u.name, roleLabel(u.role), u.numeroEmpleado?.let { "#$it" }, u.department?.name).joinToString(" · ")
    }
    val empleadoOptions = state.filteredEmpleados.map { u ->
        u.id to listOfNotNull(u.name, u.numeroEmpleado?.let { "#$it" }, u.puesto, u.department?.name).joinToString(" · ")
    }
    ...
    Column(
        ...
        AppSurfaceCard(modifier = Modifier.fillMaxWidth()) {
            ...
            AppSearchField(state.empleadoQuery, viewModel::onEmpleadoQueryChange, "Buscar por nombre, número, puesto…")
            Spacer(Modifier.height(8.dp))
            SimpleDropdownField(
                label = "Empleado",
                value = state.selectedEmpleadoId,
                options = empleadoOptions,
                onSelect = viewModel::onEmpleadoChange,
                modifier = Modifier.fillMaxWidth(),
            )
            Spacer(Modifier.height(10.dp))
            OutlinedTextField(
        ...
        AppSurfaceCard(modifier = Modifier.fillMaxWidth()) {
            ...
            AppSearchField(state.jefeQuery, viewModel::onJefeQueryChange, "Buscar administrador, gerente o jefe…")
            Spacer(Modifier.height(8.dp))
            SimpleDropdownField(
                label = "Jefe de área (encargado)",
                value = state.selectedEncargadoId,
                options = jefeOptions,
                onSelect = viewModel::onEncargadoChange,
                modifier = Modifier.fillMaxWidth(),
            )
            Spacer(Modifier.height(10.dp))
            OutlinedTextField(
